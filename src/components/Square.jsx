...
const Square = ({ value, classOption, id, onClick }) => {
  const image = (
    <img src={imageMap[value]} className="piece-image" id={value} alt={value} />
  );
  return (
    <button className={classOption} id={`square-${id}`} onClick={onClick} value={id}>
      {image}
    </button>
  );
};
